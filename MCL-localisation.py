...
#from uefs_simulator import Vehicle, Lidar  # UEFS simulator components
...
def main():
    pass
    """
    Having issues with the uefs import, run using own inputs till fix
    # Initialize UEFS simulator
    # vehicle = Vehicle()
    # lidar = Lidar()
    true_pose = (5.0, 5.0, 0.0)
    delta_x, delta_y, delta_theta = 0.1, 0.0, 0.01
    observations = [
        (1, 5.0, 0.5),
        (2, 10.0, 1.2)
        ]
    
    # Known map (landmark positions)
    map_landmarks = {
        1: (10.0, 10.0),
        2: (10.0, 20.0),
        3: (20.0, 10.0),
    ...
    mcl = MCL(
        num_particles=1000,
        map_size=(30, 30),  # Width and height of the map
        initial_pose=(5.0, 5.0, 0.0)  # Initial guess (x, y, theta)
    )
    
    # Simulation loop
    for step in range(1000):
        # Get true pose from simulator (for visualization only - not used in MCL)
        true_pose = vehicle.get_pose()
        
        # Get control inputs (odometry)
        delta_x, delta_y, delta_theta = vehicle.get_odometry()
        
        # Get sensor observations (landmark detections)
        ...
        print(f"Step {step}: Estimated Pose = {estimated_pose}")
        
        # Visualization
        mcl.visualize(true_pose=true_pose, map_landmarks=map_landmarks)
        
        # Break if converged
        if step > 50 and np.std([p.x for p in mcl.particles]) < 0.1 and np.std([p.y for p in mcl.particles]) < 0.1:
